    ...
    public class UserService : IUserService
    {
        ...
        public async Task UpdateReturnedDateAsync(BorrowingRecord Book, int id)
        {
            ...
            if (result_update != null)
            {
                ...
                result_update.Status = Book.Status;
                await _dbContext.SaveChangesAsync();
            }
        }
        ...
        public async Task<List<BorrowingRecord>> GetCheckedOutBooksAsync()
        {
            var result = await _dbContext.BorrowingRecord
                .Where(record => record.Status == "Checkout")
                .OrderByDescending(record => record.Id)
                .Include(record => record.Book)
                .Include(record => record.User)
                .ToListAsync();
        ...
        {
            var borrowingRecord = await _dbContext.BorrowingRecord
                .Include(br => br.Book)
                .Include(br => br.User)
                .FirstOrDefaultAsync(br => br.Id == id);
            return borrowingRecord;
        }
        public async Task<bool> DeleteBorrowingRecordAsync(int recordId)
        {
            var record = await _dbContext.BorrowingRecord.FindAsync(recordId);
            if (record != null)
            {
            ...
            return false; // Trả về false khi không tìm thấy bản ghi
        }
